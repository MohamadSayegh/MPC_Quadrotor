...
# ocp.subject_to( slack_0 >= (sumsqr(p[0:2] - p0)  -  1.2*r0**2 >= -slack_0))
# ocp.subject_to( slack_1 >= (sumsqr(p[0:2] - p1)  -  1.2*r1**2 >= -slack_1))
# ocp.subject_to( slack_2 >= (sumsqr(p[0:2] - p2)  -  1.2*r2**2 >= -slack_2))
# ocp.subject_to( slack_3 >= (sumsqr(p[0:2] - p3)  -  1.2*r3**2 >= -slack_3))
# ocp.subject_to( slack_4 >= (sumsqr(p[0:2] - p4)  -  1.2*r4**2 >= -slack_4))

# ocp.add_objective(1000*(slack_0 + slack_1 + slack_2 + slack_3 + slack_4))
...
ocp.subject_to((sumsqr(ocp.at_tf(x) - pf[0])) <= slack_tf_x)
ocp.subject_to((sumsqr(ocp.at_tf(y) - pf[1])) <= slack_tf_y)
ocp.subject_to((sumsqr(ocp.at_tf(z) - pf[2])) <= slack_tf_z)
...
ocp.add_objective(70*ocp.integral(sumsqr(p-pf)))
...
while True:
    ...
    if (error < clearance and error_v < clearance_v) or i == Nsim:
