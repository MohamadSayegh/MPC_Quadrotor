...
obj = 10*(sx_sol[0] + sy_sol[0] + sz_sol[0]) + 10*(sumsqr(current_X[0:3] - p_final)) + (1e-6)*(sumsqr(ux_sol[0] + uy_sol[0] + uz_sol[0] + uphi_sol[0]))
...
while True:
    ...
    if new_path_not_needed == False:
        if (abs(obj - obj_old) < 1e-2) and (error >= local_min_clearance):
        # if (abs(x_hist[i,0] - x_hist[i-1,0]) < local_min_clearance) and (abs(y_hist[i,0] - y_hist[i-1,0]) < local_min_clearance) and (abs(z_hist[i,0] - z_hist[i-1,0]) < local_min_clearance) and (abs(phi_hist[i,0] - phi_hist[i-1,0]) < local_min_clearance):
            ...
            if pathx[-1] == xf and pathy[-1] == yf:
                new_path_not_needed = True
...
def animate(i):
    ...
    T.set_data(trajectory_x[i,:],trajectory_y[i,:])
    
    return [O1,O2,O3,O4,O5,P,Px,T]  
